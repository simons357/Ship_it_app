...
class Pr24ClosureReviewTests(unittest.TestCase):
    def test_pages_do_not_close_ns(self):
        ...
        self.assertIn("written derivation available", bound)
        self.assertIn("independent specialist review pending", bound)
        self.assertIn("does **not** perform the 9B", bound)
        self.assertIn("normalized torus", bound.lower())
        self.assertIn(r"w\neq", bound)
        self.assertIn(r"\alpha>0", bound)
    ...
    def test_family_kills_the_box_on_the_live_evaluator(self):
        ...
        self.assertTrue(payload["symbolic"]["all_ok"])
        self.assertTrue(payload["symbolic"]["three_shear_ok"])
