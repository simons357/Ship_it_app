...
class WildlifeOnly(unittest.TestCase):
    ...
    def test_app_never_claims_species(self):
        ...
        self.assertIn("LISTEN TO THE FIELD", HTML)
        self.assertIn("ON AIR", HTML)
        self.assertIn('id="onAir"', HTML)
        self.assertRegex(HTML, r'id="onAir"[^>]*hidden')
        css = (ROOT / "app/css/app.css").read_text()
        self.assertIn(".on-air[hidden]", css)
        self.assertIn("WHAT THE WILD IS SAYING", HTML)
        self.assertIn("liveMeter", HTML)
        ...
        self.assertIn("storyListen", HTML)
        self.assertIn("GREEN SCREEN · READ TO THE CHILDREN", HTML)
        self.assertIn("bedtime.html", HTML)
        stories = (ROOT / "app/js/stories.js").read_text()
        self.assertIn("The Wind and the Sun", stories)
        self.assertIn("tonightStory", stories)
        self.assertIn("The Nightingale", stories)
        self.assertIn("Diamonds and Toads", stories)
        self.assertIn("The Buried Moon", stories)
        self.assertIn("older and rarer", HTML)
        lessons = (ROOT / "app/js/lessons.js").read_text()
        self.assertIn("For whoever is teaching", lessons)
        self.assertIn("todayLesson", lessons)
        self.assertNotIn("Professor", lessons)
        bedtime = (ROOT / "app/bedtime.html").read_text()
        self.assertIn("#00FF00", bedtime)
        self.assertIn("tonightStory", bedtime)
        self.assertIn("todayLesson", bedtime)
        self.assertIn("I’LL TEACH", bedtime)
        self.assertNotIn("Where the Wild Things", stories + bedtime + lessons)
        self.assertNotIn("Savannah marsh", HTML)
        self.assertNotIn("Party mode", HTML)
        self.assertNotIn("Clapper rail", HTML)
        self.assertIn("listen-pill", HTML)
        self.assertNotIn('class="record-btn start"', HTML)
        self.assertIn("session-origin", (ROOT / "app/js/app.js").read_text())
        self.assertIn(".session-origin", css)
        self.assertIn("UNKNOWN stays UNKNOWN", APP)
        self.assertIn("startRecord", APP)
        self.assertIn("stopRecord", APP)
        self.assertIn("listener.weather", (ROOT / "app/js/plugins.js").read_text())
        self.assertIn("listener.search", (ROOT / "app/js/plugins.js").read_text())
        self.assertNotIn("open-meteo", (ROOT / "app/js/weather.js").read_text())
    ...
    def test_first_sound_is_unknown_not_contributed(self):
        ...
        self.assertIn("recordHomeHTML", APP)
