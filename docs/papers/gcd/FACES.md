# Inverse-GCD / Q6 has several faces. They are not one mix paper.

Do **not** glue this book to Paper2 NS, Ring SND, RH, Goldbach, Clay, or SFE.
Do **not** overwrite [`04_q6_inverse_gcd.pdf`](04_q6_inverse_gcd.pdf).
Do **not** invent a second `GCD_SPECTRAL_ATTRACTOR_MAGNUM.tex`. Hash-prefixes
`224b718b3_GCD_SPECTRAL_ATTRACTOR_MAGNUM` and `f246f9e41_GCD_SPECTRAL_ATTRACTOR_MAGNUM` are **aliases** of the archived mix TeX.

**Usable theory H** on this branch remains
`HN = D^((-1)/2)*Qtilde*D^((-1)/2)`.
Withdrawn: all-\(N\) floor \(\lambda_{\min}>-1/2\); dark-state
\(\Leftrightarrow\) Goldbach; NS from these matrices.
Letters collide: Q6 \(H_N\) \(\neq\) Paper2 \(H_N[a]\) \(\neq\) FRA \(H\).

| Face | File | What it is |
|---|---|---|
| Controlling public PDF (21 August 2026) | [`04_q6_inverse_gcd.pdf`](04_q6_inverse_gcd.pdf) | Zenodo [10.5281/zenodo.22050962](https://doi.org/10.5281/zenodo.22050962). Title *The Inverse-GCD Operator \(Q_N\) (Q6): Definitions, a Restricted Rayleigh Bound, and an Exploratory Spectral Programme.* Face *Corrected preprint — August 2026*. 4 pages, 238 962 bytes. SHA-256 `a239112289a1a150d7a7a2212ec7f3649382b81542b49d5d2300f8611df3d6b1`. **Do not overwrite.** |
| Frankie MAGNUM TeX | **alias — not re-filed** | Base44 `224b718b3_GCD_SPECTRAL_ATTRACTOR_MAGNUM.tex` and `f246f9e41_GCD_SPECTRAL_ATTRACTOR_MAGNUM.tex` (each HTTP **302** then **200**, 37 366 bytes). SHA-256 `f41194c76cf4…`. `\title` *The GCD Spectral Attractor*; `\date{May 25, 2026}`. **Same bytes** as [`docs/archive/gcd-spectral-attractor-2026-05/Simons_GCD_Spectral_Attractor_Unified.tex`](../../archive/gcd-spectral-attractor-2026-05/Simons_GCD_Spectral_Attractor_Unified.tex). Bare name without hash was **403**. Receipt: [`docs/archive/gcd-spectral-attractor-2026-05/GCD_SPECTRAL_ATTRACTOR_MAGNUM.ALIAS.md`](../../archive/gcd-spectral-attractor-2026-05/GCD_SPECTRAL_ATTRACTOR_MAGNUM.ALIAS.md). **Archive only.** **Not** the August Q6 PDF (`a2391122…`). Clay **NOT CLAIMED**. Drive said both prefixes **gone**; git already has the mix TeX. Two receipts, one SHA. Packet [`docs/packets/MISSING-FIFTEEN-RECOVERY-AUDIT-2026-08-25.md`](../../packets/MISSING-FIFTEEN-RECOVERY-AUDIT-2026-08-25.md). |
| `GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026` | **not received** | Filename-only drop 25 Aug 2026. Extension unknown. SHA none. Base44 bare name + common extensions: HTTP **302** then **403**, 0 bytes. Zenodo exact title search: **0** hits. **Not** Q6 PDF `a2391122…`. **Not** MAGNUM mix `f41194c7…`. **Not** Paper1 UPDATED / PAPER_A / PAPER_B. Do **not** invent. Receipt: [`GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026.MISSING.md`](GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026.MISSING.md). |
| Gap 1 Reconciliation handoff (8 June 2026) | [`GAP1_RECONCILIATION_HANDOFF.md`](GAP1_RECONCILIATION_HANDOFF.md) | Base44 `907cc125a_GAP1_RECONCILIATION_HANDOFF.md`: HTTP **302** then **200**, 8 147 bytes. SHA-256 `5c2f4994ea44f589b50502a448ee85f7f4ce1c1ec9290d5761ca1bb7ce8f6960`. Title *Gap 1 Reconciliation — Handoff to Grok / Claude*. Two operators \(1/(\mathrm{gcd}\sqrt{ij})\) vs \(\mu(i/g)\mu(j/g)g/\sqrt{ij}\) are **NOT identical**. Numerical \(\lambda_{\min}/\log N\) toward \(-1/(2\pi)\) is **NOT a theorem**. Step F / Fujii seam **OPEN**. Do **not** invent the 10–20 line closure. Do **not** claim \(C\approx 0.04706\) completes a proof. Q6 PD stands as inverse-GCD arithmetic; all-\(N\) floor **withdrawn**. Source “NS unconditional” / “RH follows” **rejected**. **Not** August Q6 PDF `a2391122…`. **Not** MAGNUM `f41194c7…`. **Not** Clay. **Not** live DA. **Not** Nav42 paint. Hash-prefix **not re-filed**. Receipt: [`GAP1_RECONCILIATION_HANDOFF.RECEIPT.md`](GAP1_RECONCILIATION_HANDOFF.RECEIPT.md). |
| May 2026 mix TeX (withdrawn) | [`docs/archive/gcd-spectral-attractor-2026-05/Simons_GCD_Spectral_Attractor_Unified.tex`](../../archive/gcd-spectral-attractor-2026-05/Simons_GCD_Spectral_Attractor_Unified.tex) | Zenodo [10.5281/zenodo.20405599](https://doi.org/10.5281/zenodo.20405599). `\title` *The GCD Spectral Attractor* (NS / RH / SFE / Clay). `\date{May 25, 2026}`. 37 366 bytes. SHA-256 `f41194c76cf4…`. **Archive only.** MAGNUM hash-prefix is this file. **Not** a duplicate of the August Q6 PDF. **Not** live DA. Clay **NOT CLAIMED**. |
| Older prize-stack PDF | Zenodo [10.5281/zenodo.20271457](https://doi.org/10.5281/zenodo.20271457) | *Ramanujan–Möbius / prime lattice*. Live title **[Claim withdrawn]**. History only. Do not cite as current. **Not** re-filed here. |

## Which face to use

- **Public / usable Q6 arithmetic:** 21 August Zenodo PDF. Definitions,
  Bridge\* on a prime pair, nonnegative cone. Do not overwrite.
- **MAGNUM TeX:** alias of the May mix TeX (SHA `f41194c7…`). Do not re-file.
  Receipt [`docs/archive/gcd-spectral-attractor-2026-05/GCD_SPECTRAL_ATTRACTOR_MAGNUM.ALIAS.md`](../../archive/gcd-spectral-attractor-2026-05/GCD_SPECTRAL_ATTRACTOR_MAGNUM.ALIAS.md).
- **May mix / attractor metaphysics:** archive only. Do not upgrade NS / RH /
  Goldbach claims. Do not import into `domain_architect/`.
- **Do not merge them into one claim.**
- **`GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026`:** **not received.** Filename is not an attachment. **Not** MAGNUM. **Not** the August Q6 PDF. Receipt [`GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026.MISSING.md`](GCD_Spectral_Dynamics_Report_Jonathan_Simons_2026.MISSING.md).
- **Gap 1 Reconciliation handoff:** arrived (SHA `5c2f4994ea44…`). Two operators **not identical**. Step F **OPEN**. **Not** a RH/NS/Goldbach closure. **Not** live DA. Receipt [`GAP1_RECONCILIATION_HANDOFF.RECEIPT.md`](GAP1_RECONCILIATION_HANDOFF.RECEIPT.md). This E8 / Q6 “spectral floor” slogan in the sibling Grok dump is **not** Q6 \(H_N\). Track C: [`docs/archive/nav-42-cbfd-2026-04/GROK-E8-NAV42-SUPERSTRUCTURE.RECEIPT.md`](../../archive/nav-42-cbfd-2026-04/GROK-E8-NAV42-SUPERSTRUCTURE.RECEIPT.md).
- **PAPER_A / PAPER_B / `GCD_Spectral_Paper1_UPDATED_*`:** pack-only, **not on this VM**. They **never lived** in Overleaf exports. Do not hunt them as Overleaf-required. Policy [`docs/packets/OVERLEAF-VS-PACK-AUDIT-2026-08-15.md`](../../packets/OVERLEAF-VS-PACK-AUDIT-2026-08-15.md). Drive Missing Fifteen: still needed via upload/paste. Zenodo lead for PAPER_B is a deposit that may carry source — **not** MAGNUM mix TeX `f41194c7…` and **not** automatically the August Q6 PDF `a2391122…`. Packet [`docs/packets/MISSING-FIFTEEN-RECOVERY-AUDIT-2026-08-25.md`](../../packets/MISSING-FIFTEEN-RECOVERY-AUDIT-2026-08-25.md).
- **April Overleaf CLAY/SERPENT mains:** **not** Q6 arithmetic. **Not received.** Receipt [`docs/archive/overleaf-2026-04/`](../../archive/overleaf-2026-04/). Clay **NOT CLAIMED**.

Sibling Frankie `SPECTRAL_UNIFICATION_PAPER.tex` is archived under
[`docs/archive/sfe-hb/`](../../archive/sfe-hb/) (June 10 overclaim, not Q6
arithmetic). `SND_GNC_BRIDGE_EXTRACTED.txt` is a **different** object. Do not
overwrite those files. June FIXED PDF (`7de9444d…`) is Paper2, not this book.
