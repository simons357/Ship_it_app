...
\begin{proposition}\label{prop:L2}
  ...
  \begin{equation}\label{eq:H1int}
    ...
    \qquad
    \e\int_0^T\norm{u_\e}_{\dot H^{1.3}}^2\,dt \le \frac{M^2}{2},
  \end{equation}
  both uniform in $\e > 0$.
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\le \e_k^{1/2}
      \left(\e_k\int_0^T\norm{u_{\e_k}}_{\dot H^{1.3}}^2\,dt\right)^{1/2}
      \left(\int_0^T\norm{\phi}_{\dot H^{1.3}}^2\,dt\right)^{1/2}\\
    &\le \e_k^{1/2}\cdot\frac{M}{\sqrt{2}}
      \cdot\norm{\phi}_{L^2_t \dot H^{1.3}} \to 0.
  \end{align*}
...
\begin{enumerate}
  ...
  \item The $Q_1$ augmentation term vanishes in the limit via the
    energy bound $\e_k\int\norm{u_{\e_k}}_{\dot H^{1.3}}^2 \le M^2/2$
