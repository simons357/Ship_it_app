...
(Zenodo \href{https://doi.org/10.5281/zenodo.20405404}{10.5281/zenodo.20405404}).
The identity \(\frac1{r^4}\partial_z(\Gamma^2)=\partial_z(\Phi^2)\)
with \(\Gamma=r\ut\) and \(\Phi=\ut/r\) is retained as algebra.
The \(\Phi\)-energy is taken in the measure \(r^3\,\dd r\,\dd z\),
in which \(\Lfour=\partial_{rr}+(3/r)\partial_r+\partial_{zz}\) is
self-adjoint and the axis boundary term of the \(r\,\dd r\,\dd z\)
calculus is absent. The headline identity is
\begin{equation*}
\tfrac12\ddt\norm{\Ph}{L^2(r^3)}^2
...
That bound is open. It is equivalent in difficulty to global regularity
for the axisymmetric-with-swirl class: it is the theorem, not a
\(\delta\)-loss. Two pillars are proved: the maximum principle for
\(\Gamma\), and global smoothness of the \(\eps\)-system.
...
Proved here: \(\Phi\)-identity; equation for \(\Ph\);
\(\Gamma\) maximum principle with the scalar equation written;
energy identity \eqref{eq:star} in \(r^3\,\dd r\,\dd z\), including the
sign of \(\divfive(u)=2\ur/r\); criticality of the strain pairing;
...
prime-lattice, \(Q_6\), or spectral-clock statements.
\end{mdframed}
...
The centrifugal source \(\frac1{r^4}\partial_z(\Gamma^2)\) in the
meridional vorticity equation is the classical obstruction
\cite{Danchin2007,CSTY2008,CSTY2009,LeiZhang2011}.

The substitution \(\Ph=\ut/r=\Gamma/r^2\) rewrites that source as
\(\partial_z(\Ph^2)\). This is algebra, not a bound
(Theorem~\ref{thm:identity}).
The difficulty moves onto \(\Ph\) and onto the radial strain \(\ur/r\).
...
only remaining hypothesis, and records why interpolation cannot buy it.
...
circulation-gradient construction is not imported.
A short, checkable identity plus an accurately named obstruction is
the competitive document; an unchecked closure is not.
...
\(\eps\norm{u}{\Hdot^{1.3}}^2\). The number \(2.6\) is not the energy
index. Duality is
\begin{equation}
  \label{eq:dual}
...
\eqref{eq:id} follows. The difference identity is the same factorization.
\end{proof}
...
\section{Pillar I: maximum principle for \(\Gamma\)}
...
Substituting \(\Ga=r\ut\) and using \eqref{eq:div3} yields the
...
for \(\Ga\). The viscous operator in \eqref{eq:Gamma} is nonpositive
on maxima (and nonnegative on minima) after the standard axisymmetric
integration by parts; equivalently, at an interior spatial maximum of
\(\Ga(\,\cdot\,,t)\) one has \(\nabla\Ga=0\) and
...
holds for smooth \(\Ph\) (expand and cancel). Divide by \(r>0\) and
extend by continuity to the axis.
\end{proof}
...
circular. We do not do this.
\end{remark}

\begin{remark}[Weak limits do not move pointwise bounds]
\label{rem:weak}
Aubin--Lions applied to \eqref{eq:U1} yields, up to subsequence,
strong convergence \(u_\eps\to u\) in \(L^2_{\mathrm{loc}}([0,T]\times\R^3)\)
and a Leray--Hopf limit. Strong \(L^2\) is enough for the convective
term distributionally. It is not enough to pass an \(\Lint\) strain
bound unless that bound is already uniform in \(\eps\).
...
\(r\to 0\), and \(\Ph=o(r^{-3/2})\) at infinity in the sense that
\(r^3\Ph\partial_r\Ph\to 0\) as \(r\to\infty\), uniformly in \(z\)
after \(z\)-integration. Then
\begin{equation}
  \label{eq:visc}
...
\end{align*}
The bracket vanishes at \(r=0\) by the factor \(r^3\) and at infinity
by the decay hypothesis. There is no axis term
\(-\int\Ph(0,z)^2\,\dd z\).
\end{proof}

\begin{theorem}[Headline energy identity]
\label{thm:star}
Along a smooth solution of \eqref{eq:Phieps} satisfying the decay of
Lemma~\ref{lem:visc},
\begin{equation}
  \label{eq:star}
  \tfrac12\ddt\norm{\Ph}{L^2(r^3)}^2
...
\begin{proof}
Multiply \eqref{eq:Phieps} by \(\Ph\,r^3\) and integrate.
The time term is the first term of \eqref{eq:star}.
The viscous term is Lemma~\ref{lem:visc}.
The fractional term is coercive on \(\Hdot^{1.3}(r^3)\) by \eqref{eq:dual}.

For advection,
\(\int(u\cdot\nabla\Ph)\Ph\,r^3
=\tfrac12\int u\cdot\nabla(\Ph^2)\,r^3\).
Integrating by parts (boundary terms vanish by the same decay as in
Lemma~\ref{lem:visc}) and using Lemma~\ref{lem:div5},
\begin{align*}
\int u\cdot\nabla(\Ph^2)\,r^3
&=-\int\Ph^2\bigl(\partial_r(r^3\ur)+\partial_z(r^3\uz)\bigr)\,\dd r\,\dd z
=-\int\Ph^2\divfive(u)\,r^3\\
&=-2\int\frac{\ur}{r}\Ph^2\,r^3,
\end{align*}
so
\(\int(u\cdot\nabla\Ph)\Ph\,r^3=-\int(\ur/r)\Ph^2\,r^3\).
The stretching term in \eqref{eq:Phieps} contributes
\(+2\int(\ur/r)\Ph^2\,r^3\).
...
\textbf{Open problem (the barrier).}
...
Let \(S:=\ur/r\), \(E:=\norm{\Ph}{L^2(r^3)}^2\),
\(D:=\norm{\gradfive\Ph}{L^2(r^3)}^2\).
The pairing is \(\int S\Ph^2\,r^3\).
The NS scaling \(u_\lambda(x,t)=\lambda u(\lambda x,\lambda^2 t)\)
gives \(\Ph_\lambda=\lambda\Ph(\lambda r,\lambda z,\lambda^2 t)\)
and \(S_\lambda=\lambda^2 S(\lambda r,\lambda z,\lambda^2 t)\).
Then
\(\int S\Ph^2 r^3\,\dd r\,\dd z\)
and \(\nu D\) have the same degree.
The natural Young split
\(\abs{\int S\Ph^2 r^3}\le\norm{S}{\Lint}E\)
returns Gronwall and \eqref{eq:B1}.
Any attempt to hide a piece in \(\nu D\) is critical on the
five-dimensional lift: an inequality
\(\abs{\int S\Ph^2 r^3}\le\delta\nu D+C_\delta(\cdots)\)
...
\item A universal constant relating coprime density to shell energies.
...
The May 2026 deposit is \cite{SimonsPhi2026}.
