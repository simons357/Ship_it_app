...
| Axisymmetric shell estimate (2026-09-12) | [`AXISYMMETRIC-SHELL-ESTIMATE.md`](AXISYMMETRIC-SHELL-ESTIMATE.md) | Door-1 pairing on the **unaugmented axisymmetric-with-swirl** class. Energy \(Z_j=\tfrac12\|P_j u\|_{L^2}^2\) stays distinct from enstrophy \(Z_j=\|\Delta_j\omega\|_{L^2}^2\). Only remainder \(T_{j\leftarrow j}\), still **OPEN**. Spectral-shift identity is bookkeeping, not Lemma★. \(\rho_j<\nu\) is not energy-budget absorption. Occupancy 1 with \(\alpha\approx 1/2\) is not depletion. Filter [`docs/domain-architect/AXISYMMETRIC-SHELL-AUDIT.md`](../../domain-architect/AXISYMMETRIC-SHELL-AUDIT.md). **Not** leftover-split strain. **Not** Paper2 T2. Clay **NOT CLAIMED**. |
