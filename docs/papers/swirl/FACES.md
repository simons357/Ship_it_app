...
| Axisymmetric shell estimate (2026-09-12; Status / Terminology / Scope lock 2026-09-13) | [`AXISYMMETRIC-SHELL-ESTIMATE.md`](AXISYMMETRIC-SHELL-ESTIMATE.md) | Door-1 pairing on the **unaugmented axisymmetric-with-swirl** class. Energy \(Z_j=\tfrac12\|P_j u\|_{L^2}^2\) stays distinct from enstrophy \(Z_j=\|\Delta_j\omega\|_{L^2}^2\). Only remainder \(T_{j\leftarrow j}\), still **OPEN**. Far-shell Young sits; requested local Young **REFUSED**. Locked **Status / Terminology / Scope**: spectral-shift identity ≠ Lemma★ ratio bound; \(\rho_j<\nu\) not shell-energy absorption; cross-scale bounds open; (A)–(C)+Step 6 candidate only; occupancy 1 with \(\alpha\approx 1/2\) is not depletion; numerics scoped to small exact disks / restricted classes. Filter [`docs/domain-architect/AXISYMMETRIC-SHELL-AUDIT.md`](../../domain-architect/AXISYMMETRIC-SHELL-AUDIT.md). **Not** leftover-split strain. **Not** Paper2 T2. Clay **NOT CLAIMED**. |
