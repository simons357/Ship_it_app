...
\textbf{SND condition (this note):} $\sup_{t\geq 0}\rho(t) \leq \rho_0$ for some $\rho_0 < 1$.
This is the \emph{upper-bound / non-concentration} packaging of shell fractions
$a_j=E_j/X$ with $X=\|u\|_{L^2}^2$.
It is \emph{not} identical to the Ring-companion lower-bound form
$\inf_t J/X_{\mathrm{enst}}\ge c_*>0$ (enstrophy shells); see the Tao panel note
\texttt{docs/math/TAO-MATH-PANEL-SND-H.md}. Both are conditional hypotheses.
...
Since $\mathcal{D}=\nu X\sum_j\lambda_j a_j$, the same bound is
...
(Theorem~\ref{thm:alpha} supplies the matching Poincar\'e \emph{lower} bound
$\mathcal{D}/X \geq \nu\cdot 4^{1/\rho_0}\cdot\rho_0$, which fixes $\alpha>0$,
but does not by itself force $\beta_N$ small.)
The needed upper bound $\mathcal{D}/X \leq C(\nu,N,R_\varepsilon)$ follows from
the finite-$N$ truncation in the $H^{2.3}$ absorbing ball.
For the threshold class, that upper bound keeps $\beta_N < \alpha\eta_N$.\qed
\end{proof}
...
\item Any Goldbach / GNC identification with SND (withdrawn; see \texttt{06\_status\_errata.tex}).
\end{itemize}
\end{mdframed}
