...
Proved: Bridge$^*$ on single prime pairs, multi-representation Goldbach
vectors, and the nonnegative cone.
...
\rowcolor{lightgreen}
Multi-representation Bridge$^*$ & \textcolor{proved}{\textbf{Proved}}\\
\bottomrule
\end{tabular}
\end{mdframed}
...
We prove Bridge$^*$ for single prime pairs $v=e_p-e_q$ and for
multi-representation vectors
$v_k=\sum_{p\in\mathcal{R}(k)}(e_p-e_{k-p})$ (even $k$): both satisfy
$R>-1/2$ on $\Qt_N$. The multi-rep case reduces to the pair case by a
nonnegative cross-term identity.
...
\begin{corollary}[Uniform pair floor]\label{cor:sharp}
For distinct primes $p,q$, $R(e_p-e_q)>-1/\sqrt{pq}\ge -1/\sqrt6\approx -0.408$.
\end{corollary}

\begin{proof}
From Theorem~\ref{thm:bridge}, $R+1/2>\tfrac12-1/\sqrt{pq}$, hence $R>-1/\sqrt{pq}$.
Distinct primes give $pq\ge 6$.
\end{proof}
...
Theorem~\ref{thm:bridge}, Theorem~\ref{thm:multirep}, and
Propositions~\ref{prop:floorfails}--\ref{prop:cone}.

\section{Bridge$^*$: multi-representation}

\begin{lemma}[Cross-term factorization]\label{lem:cross}
For distinct primes $p\neq q$ and $r\neq s$,
\[
\widetilde Q_N(p,r)-\widetilde Q_N(p,s)-\widetilde Q_N(q,r)+\widetilde Q_N(q,s)
=\Bigl(p^{-1/2}-q^{-1/2}\Bigr)\Bigl(r^{-1/2}-s^{-1/2}\Bigr).
\]
In particular, if $p<q$ and $r<s$, the cross term is strictly positive.
\end{lemma}

\begin{proof}
Each off-diagonal entry is $(\,\cdot\,)^{-1/2}$ for distinct primes, so the
left-hand side factors as claimed.
\end{proof}

\begin{theorem}[Bridge$^*$, multi-representation]\label{thm:multirep}
Let $k\le N$ be even and
\[
\mathcal{R}(k)=\{p\text{ prime}:2\le p<k/2,\ k-p\text{ prime}\}.
\]
Set $v_k=\sum_{p\in\mathcal{R}(k)}(e_p-e_{k-p})$ (indices $\le N$).
If $v_k\neq 0$, then
\[
R(v_k)=\frac{v_k^\top\Qt_N v_k}{\|v_k\|_2^2}
\;>\;-\frac12.
\]
\end{theorem}

\begin{proof}
Distinct unordered Goldbach pairs for fixed $k$ are pairwise disjoint as
sets of prime sites: a shared prime forces the same complementary prime.
Write $v_k=\sum_{a=1}^{m}v_a$ with $v_a=e_{p_a}-e_{q_a}$, $p_a<q_a=k-p_a$.
Then $\|v_k\|_2^2=\sum_a\|v_a\|_2^2=2m$ and
\[
v_k^\top\Qt_N v_k
=\sum_a v_a^\top\Qt_N v_a
+2\sum_{a<b}v_a^\top\Qt_N v_b.
\]
Each diagonal block equals $\|v_a\|_2^2\,R(v_a)$ with $R(v_a)>-1/2$ by
Theorem~\ref{thm:bridge}. Each cross block equals the factorization in
Lemma~\ref{lem:cross} with $p_a<q_a$ and $p_b<q_b$, hence is positive.
Therefore
\[
R(v_k)
\ge\frac{\sum_a\|v_a\|_2^2\,R(v_a)}{\sum_a\|v_a\|_2^2}
\ge\min_a R(v_a)
\;>\;-\frac12.
\]
\end{proof}
...
\begin{remark}
The worst multi-rep Rayleigh through $N=200$ coincides with a single-pair
case (e.g.\ $k=8$, pair $(3,5)$, $R\approx-0.183$). Richer multi-rep
vectors sit strictly higher because of positive cross terms.
\end{remark}
...
\bibitem{ZenodoQ6}
J.R.~Simons,
\textit{Earlier Q6 draft} (archive),
