...
\textbf{WITHDRAWN / ARCHIVE ONLY (status tables).}
Later sections of this manuscript mark ``Main Theorem / Statement~(B) Proved''
and ``large data $[\mathrm{SND}]$ resolved.'' Those closures are \textbf{false}
as unconditional Clay claims (Theorem~H remains SND-C under $X\le M$).
Corrected public record: \texttt{docs/papers/submit/02\_ring\_lemma\_snd\_conditional.tex}
and \texttt{06\_status\_errata.tex}. Navier--Stokes is \textbf{not} solved.}}
\end{center}
\vspace{0.75em}
\maketitle
