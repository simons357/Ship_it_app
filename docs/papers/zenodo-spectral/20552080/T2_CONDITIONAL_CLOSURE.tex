...
\textbf{ERRATA LOCK (2026-09-12).}
Viscosity packaging uses $\mathcal{D}\ge\nu\cdot 4^{1/\rho_0}\rho_0 X$ and
$\alpha=2\nu\cdot 4^{1/\rho_0}\rho_0$ (not $\nu^2$).
SND$\equiv$GNC / Goldbach packaging from older drafts is \textbf{withdrawn};
see \texttt{docs/papers/submit/03\_t2\_shell\_flux\_gronwall.tex} and
\texttt{06\_status\_errata.tex}. Conditional on SND only; NS not solved.}}
\end{center}
\vspace{0.5em}
\maketitle
...
states: when $\rho(t)\leq\rho_0$, one has
$\|\nabla u\|_{L^2}^2\geq 4^{1/\rho_0}\cdot\rho_0\cdot X(t)$, hence
$\mathcal{D}(t)=\nu\|\nabla u\|_{L^2}^2\geq\nu\cdot 4^{1/\rho_0}\cdot\rho_0\cdot X(t)$.
(The factor is $\nu$, not $\nu^2$: Poincar\'e is viscosity-free on $\|\nabla u\|^2/X$.)
Since $\mathcal{D}=\nu X\sum_j\lambda_j a_j$, one gets
$2\nu\sum_j\lambda_j a_j=2\mathcal{D}/X\geq 2\nu\cdot 4^{1/\rho_0}\cdot\rho_0=\alpha_F$.
The shell eigenvalue lower bound $\lambda_j\geq 4^j$ on $\mathbb{T}^3$
and the normalization $\sum_j a_j = 1$ give the stated $\ell^1$ dissipative control.
\qed
\end{proof}
...
\]
This is an \emph{upper} bound on $\mathcal{D}/X$.
(Theorem~\ref{thm:alpha} supplies the Poincar\'e \emph{lower} bound
$\mathcal{D}/X \geq \nu\cdot 4^{1/\rho_0}\cdot\rho_0$, which fixes $\alpha>0$,
but does not by itself force $\beta_N$ small.)
The needed upper bound follows from the finite-$N$ truncation in the
$H^{2.3}$ absorbing ball. For the threshold class, that keeps
$\beta_N < \alpha\eta_N$.\qed
\end{proof}
...
= \frac{2\nu\cdot 4^{1/\rho_0}\cdot\rho_0\cdot\eta_N}{2C\cdot N^{1/2}\cdot R_\varepsilon\cdot\nu^{-1/2}}
= \frac{\nu^{3/2}\cdot 4^{1/\rho_0}\cdot\rho_0\cdot\eta_N}{C\cdot N^{1/2}\cdot R_\varepsilon}.
\]
\underline{Withdrawn weld:} the identification SND $\equiv$ GNC and any Goldbach
corollary from $\kappa^*$ are \textbf{not claimed} (see
\texttt{docs/papers/submit/06\_status\_errata.tex}). The displayed $\kappa^*$ is
only a formal rearrangement of the T2 constants under SND.
\end{corollary}
...
\item Formal $\kappa^*$ from T2 constants is computable; \textbf{no Goldbach claim}.
\end{enumerate}
