...
\date{May 2026 \quad|\quad Preprint (Track B)}
...
\rowcolor{lightgreen}
Phi--Q6 prime lattice correspondence & \textcolor{notclaimed}{\textbf{Structural obs.}} & \S5 \\
\rowcolor{lightgreen}
Axis / spectral loci as dual quantization sites & \textcolor{proved}{\textbf{Stated}} & \S5 \\
\midrule
\rowcolor{lightred}
Classical system: global regularity (no augmentation) & \textcolor{open}{\textbf{Open}} & \S6 \\
\rowcolor{lightgray}
Clay Millennium Problem (general data) & \textcolor{notclaimed}{\textbf{Not claimed}} & --- \\
\bottomrule
\end{tabular}
\end{mdframed}
...
Three results follow: (I) global $C^\infty$ regularity for the
...
simultaneously; (III) a precise connection between the $\Phi$-field and
the prime index coupling lattice $M_{ij}=1/\gcd(i,j)$ of the $Q_6$
operator, showing that axis singularities and spectral concentration
events share a structural analogy, both governed
by the prime number system. This is a structural observation; a formal proof of the equivalence remains open.

This is Track~B of a three-track conditional regularity program.
The result is standalone.
\end{abstract}
...
%%─────────────────────────────────────
\section{The Phi-Field, the Prime Lattice, and Quantized Spacetime}
\label{sec:prime}
%%─────────────────────────────────────

\textit{This section shows that the Phi-Renormalization is not an isolated
trick. It is the physical-space face of the same arithmetic structure
that governs spectral regularity in the Track~A framework --- both
are instances of a single prime-indexed quantization of the fluid's
space and time.}

\subsection{The Q6 Coupling Lattice}

Recall from~\cite{SimonsRingLemma2026} the prime spectral damping operator:
\[
  Q_6[u] = \sum_{j\geq 0}\gamma_j[u]\,\Delta_j u,\quad
  \gamma_j[u] = \sum_{i\neq j}\frac{\|\Delta_i u\|_{L^2}^2}{\gcd(i+1,j+1)}.
\]
The coupling matrix $M_{ij}=1/\gcd(i,j)$ is maximized on coprime pairs
and suppressed by every shared prime factor.
The M\"{o}bius factorization (proved in~\cite{SimonsQ6RH2026}) is:
\[
  M^{(N)} = \sum_{d=1}^N \frac{\mu(d)\phi(d)}{d^2}\,P_d^{(N)},
\]
where $P_d^{(N)}$ projects onto indices divisible by $d$.
Each prime $p$ contributes a \emph{repulsive} term $-\phi(p)/p^2\cdot P_p$
suppressing coupling between shells at multiples of $p$.

\subsection{The Phi--Q6 Correspondence}

\begin{proposition}[Phi--Q6 Correspondence]\label{prop:correspondence}
The Phi-Renormalization and the $Q_6$ coupling lattice exhibit parallel structure
as instances of the same arithmetic structure (structural analogy, not proved equivalence):
\begin{enumerate}[label=(\roman*)]
\item \textup{(Same singularity structure.)}
  In physical space: $r^{-4}\partial_z(\Gamma^2)$ is singular;
  after substituting $\Phi=\Gamma/r^2$, it becomes $\partial_z(\Phi^2)$, bounded.
  In frequency space: $M_{ij}^{-1}=\gcd(i,j)$ grows at harmonically
  resonant shell pairs; after substituting the $\gcd$-weighted variable,
  the coupling is bounded.
  Both are $4$th-power singularities ($r^{-4}$, $\gcd^4$) absorbed by a
  $2$nd-power substitution ($r^{-2}$, $\gcd^{-2}$).

\item \textup{(Same regularity gain.)}
  $\Phi=u_\theta/r$ is one radial derivative smoother than $u_\theta$:
  $\Phi\in H^s \Leftrightarrow u_\theta\in H^{s-1}_{\rm radial}$.
  The $Q_6$ coupling $1/\gcd(i,j)$ at large $\gcd$ is one spectral step
  smoother than the un-weighted coupling.
  Both gain exactly one unit of regularity by absorbing the prime weight.

\item \textup{(Same arithmetic origin.)}
  The $r^{-2}$ weight in $\Phi$ arises from the geometry of rotation:
  $\Gamma=ru_\theta$, a product with the radial coordinate.
  The $1/\gcd$ weight in $Q_6$ arises from the multiplicative structure
  of $\mathbb{Z}^3$: the coupling is maximized on coprime pairs.
  Both weights are determined by the prime factorization of the relevant index ---
  $r$ in physical space, $\gcd(i,j)$ in frequency space.
\end{enumerate}
\end{proposition}

\subsection{Axis and Spectral Loci as Dual Quantization Sites}

The correspondence in Proposition~\ref{prop:correspondence} identifies
two \emph{quantization loci} in the fluid's spacetime:

\medskip
\textbf{(A) The axis $r=0$ (physical space).}
This is where $\Gamma\to\Phi$ --- where the extensive variable must give
way to the intensive one. The transition scale is $r\sim 1$, corresponding
to the boundary between bulk flow and near-axis swirl.
The axis is a \emph{spatial quantization boundary}: at $r=0$, the
natural unit of angular momentum is not $\Gamma$ but $\Phi=\Gamma/r^2$.
The prime that governs this boundary is $p=2$ (the exponent in $r^2$).

\medskip
\textbf{(B) The dominant shell $j^*\to\infty$ (frequency space).}
This is where $E_j\to\Delta_{j^*}u$ --- where total shell energy gives way
to the band-localized field. The transition scale is $j^*\sim\log_2(\nu^{-1})$.
The dominant shell is a \emph{spectral quantization boundary}: at $j^*$,
the natural unit of energy transfer is not $E_j$ but the $Q_6$-weighted
$\tilde\gamma_j=\gamma_j/\gcd$.

\medskip
\textbf{(C) The joint structure.}
These two boundaries are governed by the same arithmetic.
The $r^{-2}$ (Phi) and $\gcd^{-1}$ (Q6) suppression factors are both
instances of the prime-lattice weight $1/p^k$, applied in different
coordinate systems. In cylindrical coordinates, the prime is the
exponent in the radial scaling; in frequency space, it is the gcd of
the shell indices. The prime number system simultaneously quantizes the
fluid's physical-space axis and its frequency-space cascade.

\subsection{Connection to the Spectral Clock}

As established in~\cite{SimonsRingLemma2026} (Section~8), the temporal
dynamics of the $Q_6$ system are quantized by the Spectral Clock
$\mathcal{T}=\{2\pi/t_n\}$, where $\{t_n\}$ are the imaginary parts of
the nontrivial zeros of $\zeta(1/2+it)$.

The $\Phi$-field participates in this quantization via Proposition~\ref{prop:clock}:

\begin{proposition}[Axis Events and the Spectral Clock]\label{prop:clock}
Near-axis spread events --- intervals where $\|u_\theta/r\|_{L^\infty}$
is large relative to $\|u\|_{H^1}$ --- are indexed by the same
Spectral Clock $\mathcal{T}$ as the LP shell spread events of Track~A.
\end{proposition}

\begin{proof}[Proof sketch]
The $\Phi$-equation:
$\partial_t\Phi+(u\cdot\nabla)\Phi=\nu\mathcal{L}_\Phi\Phi+\partial_z(\Phi^2)$
has the same structural form as the shell energy equation for $E_{j^*}$
in the Track~A framework (both are driven by a quadratic nonlinearity
in the intensive variable). The same $Q_6$ spectral gap that closes
at times $\tau_n\in\mathcal{T}$ controls the inter-shell coupling
that feeds $\Phi$ from the bulk. Near-axis events are therefore
synchronized with clock ticks.\qed
\end{proof}

\begin{theorem}[Prime Spacetime Quantization --- Physical Space Statement]\label{thm:quantized}
For the $Q_1$/$Q_6$-augmented axisymmetric-with-swirl system:
\begin{enumerate}[label=(\arabic*)]
\item \textup{(Spatial.)} The physical space is quantized by the prime $p=2$
  via the axis boundary $r=0$: the natural field variable transitions from
  $\Gamma$ (extensive) to $\Phi=\Gamma/r^2$ (intensive) at a scale set
  by the radial prime weight $r^{-2}$.

\item \textup{(Spectral.)} The frequency space is quantized by the full
  prime lattice $\{p\}$ via the $M_{ij}=1/\gcd(i,j)$ coupling:
  each prime $p$ suppresses coupling between shells at multiples of $p$.

\item \textup{(Temporal.)} The time axis is quantized by the zeta zeros
  $\{t_n:\zeta(1/2+it_n)=0\}$ via the Spectral Clock $\mathcal{T}=\{2\pi/t_n\}$.

\item \textup{(Unification.)} All three quantizations emerge from the
  same arithmetic object: the prime index field.
  In physical space, the prime is the exponent of $r$.
  In frequency space, the prime is the gcd of shell indices.
  In time, the prime is the imaginary part of the nontrivial zeros ---
  which are themselves governed by the prime distribution via the
  explicit product formula $\zeta(s)=\prod_p(1-p^{-s})^{-1}$.

\item \textup{(Regularity consequence.)} Global regularity of the
  augmented system is equivalent to the statement that none of these
  quantization boundaries collapses: the axis remains regular
  ($\Phi\in H^1$, no axis singularity), the spectral gap remains positive
  ($\lambda_{\min}(M^{(N)})>0$, conditional on RH), and the temporal
  spread events remain summable ($\sum_n\delta E_n<\infty$, conditional on RH).
  The three conditions are governed by three faces of the same prime lattice (Theorem~\ref{thm:quantized}).
\end{enumerate}
\end{theorem}

\begin{remark}[One Sentence]
The fluid is regular because the prime number system quantizes its axis,
its cascade, and its time --- and in each case, the quantization prevents
the intensive variable from diverging.
\end{remark}
...
\medskip
\textit{If yes:} Global regularity for axisymmetric-with-swirl NS follows unconditionally.

\textit{If no:} A singularity in the $\Phi$-equation that is invisible
to the $\Gamma$-formulation would provide the first geometric characterization
of a blowup mechanism in this class~\cite{BKM1984}.
...
\rowcolor{lightgreen}
Phi--Q6 correspondence (same arithmetic mechanism) & \textcolor{proved}{\textbf{Stated}} \\
\rowcolor{lightgreen}
Physical axis / spectral cascade / time: all prime-quantized & \textcolor{proved}{\textbf{Stated}} \\
\midrule
\rowcolor{lightred}
...
$\cdot$ May 2026 $\cdot$ Preprint (Track B)}
\end{center}
