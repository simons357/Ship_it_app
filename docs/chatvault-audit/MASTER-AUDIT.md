...
The canonical ChatVault build is Base44 app `6a58e103fedcde66a0a7710e` (preview host `https://preview--6a58e103fedcde66a0a7710e.base44.app/`, HTML title **ChatVault — AI conversation vault**, tagline **OS for your AI**). This designation is based on Jonathan’s identification on 24 August 2026 that Claude built the original last year and the parked Base44 app is the latest; live HTTP probes; SHA-256 of the preview JS/HTML/CSS bundles; side-by-side comparison with the two other named Base44 records; browser exercise of vault/ingest/search/tags/books/dashboard/export/disclaimer; and a JSON export of one conversation. Version **v1.19 is unverified**. The following limitations remain: the Claude-origin source is still unlocated and must be preserved as historical baseline, not overwritten; the intended `ChatVaultEntry` research schema is absent from the Base44 app; the public `*.base44.app` host 404s; the editor preview is a login wall; the preview host is unauthenticated and writable; exported `raw_content` was null on a demo record; artifacts extraction is empty; there is no bulk export; Base44 source is not in git; security, privacy, Apple, and patent reviews are not passed.
...
**Phase 1 identification (24 August 2026):** Jonathan stated that Claude made the original last year, and that the one parked in Base44 looks like the latest. That matches this audit: Claude-origin = `HISTORICAL-BASELINE` (preserve, do not delete); Base44 `6a58e103fedcde66a0a7710e` = latest runnable product look (`CANONICAL-CANDIDATE`). The Claude files are still not in this workshop. Export them when found. Do not rebuild Base44 on top of them.
