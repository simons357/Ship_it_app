# Cool-check — aligned with Tao / ARCHON verdict

**Locked:** Navier–Stokes is **not** solved. Unconditional SND for large data is open.  
Theorem H ≠ unconditional SND (it is SND-C under \(X\le M\)).  
\(H_N\) ≠ Theorem H; solving \(H_N\) does not solve NS.  
Old ARCHON briefing “everything else is solid / Statement (B)” is **rejected**.

| Source | Status |
| --- | --- |
| `docs/math/TAO-MATH-PANEL-SND-H.md` | Honest — NS not solved |
| `docs/math/ARCHON-NS-FINAL-REVIEW-PANEL.md` | Rejects Statement (B); keystone fails |
| Submit TeX `01`–`07` | No Statement (B); conditional / operator-only claims |
| `06_status_errata` + `ARCHIVE-FIX` | Withdraw overclaim DOIs |
| Bridge\* / \(H_N\) scripts | Proved pair + multi-rep; refute full-spectrum / −3/14 universal |
| `route_c_gap_a_verify.py` | Killed archived Lemma A; \(R(v_{\mathrm{alt}})\not\to -1/(2\pi)\) |
| `docs/math/NS-EXTRACTION-LEDGER.md` + `route_n_shell_floor_probe.py` | Route N: convexity true; shellwise principal blocks of \(\widetilde Q\)/\(H\) clear \(-1/2\) in probe (LEAD); **NS still not solved** |

| `docs/math/ns_attacks/` five-lane Lemma★ probes | **OPEN** — K=0 killed; Lemma★/C* survive numeric only; **NS still not solved** |

**Zenodo archive-only (stamp notice, do not delete):**  
`20405526`, `20269843`, `20405589` (or replace with proper Q6), `20405593`, `20552171`, `20552400`

**Missing June 10 merge file:** still not in repo/Zenodo — does not change the verdict on the papers we have.
