...
| Tip commit | `d75460a5904342debd0c241f954b554f0bfb94e8` (locator); prior SoT tip `bd374ed0feb802a3cffdcdb33bda92da046f24c3` |
| Tip subject | Add FIVE_LANE_PACK_LOCATOR for Lemma★ SoT inventory |
...
| **HH→L attack doc** | `/workspace/docs/math/ns_attacks/ATTACK_3_BONY_HH_L.md` |
| **HH→L script** | `/workspace/scripts/ns_attacks/attack3_bony_hh_l.py` |
| **Near-shell + HH→L search** | `/workspace/scripts/ns_attacks/lemma_star_near_shell_search.py` |
...
Code aliases in `stokes_moments.py`: `E` \(=\|v\|_2^2\), `Tc` \(=T_c\), `Ds` \(=\mathcal{D}_s\), `ratio_R_star_shape` \(=\mathcal{R}_\star\).

---

## HH→L statement (from Attack 3 + status)

**Target (diagnostic, not a kill criterion alone):** partition the bilinear driving \(T_c\) into Bony channels HH / HL / LL. HH→L is the channel that blocks a clean product bound toward
...
**Lock:** HH→L splits can **identify** a mechanism; only the **complete signed** \(T_c\) enters the ★ kill criterion. The analytic reason that signed triads cannot make \(\mathcal{R}_\star\) arbitrarily large **is NOT written**.

Pointers:
- `/workspace/docs/math/ns_attacks/ATTACK_3_BONY_HH_L.md`
- `/workspace/docs/math/ns_attacks/LEMMA_STAR_SHAPE_FORM.md` (HH→L caution + proof-would-need)
- `/workspace/docs/math/ns_attacks/PROOF_LemmaStar_STATUS.md` (gap row: \(|T_c|\le C\|v\|_2 X^{3/2}\))
...
- `Hyp-ST*`, `Hyp-Lat*`, `Lat-Emb*` as standalone docs — **no hits** (HH→L content lives under Attack 3 / Lemma★ SoT above)
