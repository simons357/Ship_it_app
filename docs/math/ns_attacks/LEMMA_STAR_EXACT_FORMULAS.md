...
| Role | File |
|------|------|
| **Canonical boxed claim** (+ viscosity packaging) | [`LEMMA_STAR_CANONICAL.md`](./LEMMA_STAR_CANONICAL.md) → aliases [`LEMMA_STAR_SHAPE_FORM.md`](./LEMMA_STAR_SHAPE_FORM.md) |
| **Exact formulas** (this file) | definitions, \(D_s\) forms, triad / \(T_c\), \(\mathcal{R}_\star\), \(\Lambda'\) |
...
The **full Lemma★ claim** (boxed geometric inequality / \(\sup\mathcal{R}_\star<\infty\)), equivalent viscosity packaging
\[
T_c(u)\le\theta\nu D_s(u)+C_0(\theta)\nu^{-1}\|u\|_2^2 Y(u),\qquad
C_{\mathrm{geom}}=4\theta\,C_0(\theta),
\]
and kill/survive table live in the canonical claim doc — **not** duplicated here:
...
- Canonical claim: [`LEMMA_STAR_CANONICAL.md`](./LEMMA_STAR_CANONICAL.md) / [`LEMMA_STAR_SHAPE_FORM.md`](./LEMMA_STAR_SHAPE_FORM.md)
...
- Code: `scripts/ns_attacks/stokes_moments.py`, `tests/test_ns_attacks_lemma_star.py`
