...
3. **Positive part vs \(T_c^2\).** \(T_c(-v)=-T_c(v)\) while \(D_s,E,Y\) are even. A universal bound on \((T_c_+)^2\) is equivalent to a universal bound on \(T_c^2\). A large negative \(T_c\) becomes a kill after \(v\mapsto -v\).
...
[`LEMMA_STAR_CANONICAL.md`](./LEMMA_STAR_CANONICAL.md) is the working statement.  
...
- [`ARCHIVE_OLDER_LEMMA_STAR_PROOF.md`](./ARCHIVE_OLDER_LEMMA_STAR_PROOF.md)
