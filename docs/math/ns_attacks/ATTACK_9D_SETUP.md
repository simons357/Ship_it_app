...
**NS not solved.** Unrestricted Lemma★ is **dead** on \(v_n\). Exact-shell \(K\le16/9\) is **CLAIMED** (written derivation; internal checks; independent specialist pending). Conventions: normalized torus; \(K\) for \(\alpha>0\), \(w\neq 0\). [`docs/ns-recovery/ATTACK-9D-FULL-SUPPORT-SCORE.md`](../../ns-recovery/ATTACK-9D-FULL-SUPPORT-SCORE.md).
