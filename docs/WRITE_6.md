...
- Right-hand side matches the local identity: a slice of \(\iint|\nabla\omega|^2\phi\), plus the same \(r^{-2}\iint|\omega|^2\) Lemma C leaves. \(\nu/8\) is a conventional slice, not a sharp constant. Parabolic scaling of the three terms agrees (\(r^{-1}\)); there is no dimensional obstruction.
...
Path-cost of one pair (sits as 1-D; not this write): [`H1-PC.md`](H1-PC.md).
Shape 3 predictive (started; not this write): [`H1-PREDICTIVE.md`](H1-PREDICTIVE.md).
