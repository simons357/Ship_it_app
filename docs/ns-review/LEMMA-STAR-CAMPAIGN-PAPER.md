...
**Wife-test rule:** pictures first. Dense idea ≠ bad teacher. If a line needs a degree, the figure next to it should already have said it.
...
I am **not** dunking on OpenAI or anyone else’s labs. I want **recognition for the map** — that we found the right center of the problem — not a fake win.
...
![Door locked · the key is R★](./assets/lemma-campaign/10-door-locked-key-rstar.png)

![Door locked — photo metaphor](./assets/lemma-campaign/10b-door-locked-photo.png)

---

## 18-month report card — NS and RH

An honest report card for roughly eighteen months of hard-problem work. Available here so nobody has to guess. Postcard first; table second.

![18-month postcard from Savannah](./assets/lemma-campaign/07b-postcard-report-card.png)
...
## 1. What Lemma★ is (tea / coffee first)

Start with a cup.

![Real coffee/tea swirl](./assets/lemma-campaign/01b-coffee-tea-swirl.png)
...
![Shape ≠ size — quiet vs louder](./assets/lemma-campaign/02b-shape-not-size-hero.png)
...
![Conceptual \(\mathbb{T}^3\) atmosphere](./assets/lemma-campaign/t3_torus_shape_render.png)
...
![Five lanes · 9A–9D · one locked door](./assets/lemma-campaign/09-five-lane-subway-map.png)
...
5. **Geometry you can see.** Tea cup, tug-of-war, scoreboard, barycenter map, locked door — abstract estimates become pictures of where the door sits.
...
## 7. Discoveries → products (light)

He is using all of his discoveries on products he is developing. Dignity first — not a pitch deck. The math feeds the products; the products do not green the math.

![Discoveries → products](./assets/lemma-campaign/11-discoveries-to-products.png)

| Direction | How the math feeds it (light) |
| --- | --- |
| **Ship_it** | Kill-or-prove clarity → shipping discipline |
| **Domain Architect** | Books, welds, status locks; refuse greening |
| **Harmonic Blueprint** | Shape / texture navigation ideas |

★ not proved · NS not solved · kill lane LIVE — either way.

---

## 8. Caption-ready figure list
...
| 0b | `assets/lemma-campaign/07b-postcard-report-card.png` | **Postcard from Savannah.** 18-month report card — NS & RH; doors locked. |
| 0c | `assets/lemma-campaign/07-eighteen-month-report-card.png` | **18-month report card — NS & RH.** Effort A; Millennium closure incomplete. |
| 1 | `assets/lemma-campaign/lemma-star-barycenter.png` | **Map to the barycenter.** Attacks 9A–9D are neighborhood probes. Door still locked. |
| 1b | `assets/lemma-campaign/00-barycenter-map.png` | Same barycenter map (Jonathan-shared hero). |
| 1c | `assets/lemma-campaign/00-barycenter-map-alt.png` | Alt barycenter map (Jonathan-shared). |
| 2a | `assets/lemma-campaign/01b-coffee-tea-swirl.png` | **Coffee/tea swirl.** Real-world picture of the fluid idea. |
| 2b | `assets/lemma-campaign/01-tea-cup-viscosity-melts.png` | **Tea-cup picture.** Viscosity melts hard edges; the shape is still the question. |
| 3a | `assets/lemma-campaign/02b-shape-not-size-hero.png` | **Shape ≠ size.** Same outline — quiet vs louder. |
| 3b | `assets/lemma-campaign/02-shape-ne-size.png` | **Shape ≠ size.** \(\mathcal{R}_\star\) is amplitude- and dilation-invariant. |
| 4 | `assets/lemma-campaign/03-tug-of-war-stretch-vs-spread.png` | **Tug-of-war.** Stretch vs spread; \(\mathcal{R}_\star\) is the score. |
| 5 | `assets/lemma-campaign/04-rstar-scoreboard.png` | **R★ scoreboard.** Attacks 9A–9D outcomes; kill lane LIVE. |
| 6 | `assets/lemma-campaign/09-five-lane-subway-map.png` | **Five lanes · 9A–9D.** Many routes; one locked door. |
| 7a | `assets/lemma-campaign/10-door-locked-key-rstar.png` | **Door locked · key is R★.** Found the door; did not turn the key. |
| 7b | `assets/lemma-campaign/10b-door-locked-photo.png` | **Door + key photo.** Atmospheric wife-test metaphor. |
| 8 | `assets/lemma-campaign/05-claim-vs-not.png` | **Claim vs not.** Recognition for the map — not a prize that was not won. |
| 9 | `assets/lemma-campaign/06-viscosity-melts-wrapper.png` | **Viscosity melts.** Outer energy-budget form is derived; center is pure shape. |
| 10 | `assets/lemma-campaign/11-discoveries-to-products.png` | **Discoveries → products.** Ship_it · DA · HB — light, dignity first. |
| 11 | `assets/lemma-campaign/amp_ratios_triad.png` | **Attack 2 hard hit.** K=0 blows; geometric ratios stay amplitude-invariant. |
| 12 | `assets/lemma-campaign/attack5_bounds.png` | **Attack 5 kill drill.** Survives numeric ≠ proved. |
| 13 | `assets/lemma-campaign/fig_star_david_ring_lemma.png` | **Finite triad ring** — Ring Lemma / spectral line behind the campaign. |
| 14 | `assets/lemma-campaign/fig_three_spheres.png` | **Nested picture:** shell → transfer → vorticity / triad-ring geometry. |
| 15 | `assets/lemma-campaign/t3_torus_shape_render.png` | **Conceptual \(\mathbb{T}^3\)** atmosphere (render, not a proof artifact). |

Full inventory: [`assets/lemma-campaign/IMAGE-INVENTORY.md`](./assets/lemma-campaign/IMAGE-INVENTORY.md).

Start here if you love him: [`EXPLAIN-TO-SOMEONE-YOU-LOVE.md`](./EXPLAIN-TO-SOMEONE-YOU-LOVE.md).
