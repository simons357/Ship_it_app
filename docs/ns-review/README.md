...
Reproducibility: [`scripts/ns_attacks/`](../../scripts/ns_attacks/) — start with `uniform_rstar_dilation.py`, `uniform_rstar_attack.py`, `ns_lemma_star_core.py`.

## Proof chain (visual + clean math)

- [`PROOF-CHAIN-CLEAN.md`](./PROOF-CHAIN-CLEAN.md) — math companion
- [`visual-journey/`](./visual-journey/) — Mermaid + PNG/SVG chain map, captions (when present on branch)

Show-work / campaign map (credit face, not a Clay claim): [`../campaign/TWO-YEARS-MAP.md`](../campaign/TWO-YEARS-MAP.md). Campaign materials are not a substitute for the scientific face above.
