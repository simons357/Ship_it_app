...
and on the magazine **per-entry content window**. One **resident superagent**,
not a swarm of staff.
...
### Type in the box (voice later)

He types. That is the mouth on this square until a real ElevenLabs clone
is wired. Stock voices are forbidden. Unanswered questions get a **help
line**, then a smile. He does not invent theorems. **Weekend Update** is
still the joke-after-news job. There is no audio in the widget.

Idle in the square: red eye, **mouth as a line**. After he types an
answer (including help with unanswered), he usually smiles. Metal teeth
stay off this box.
...
- In the magazine square: type the answer. Smile after the typed line.
...
- Call `val8000_speak.py` from the magazine UI. Autoplay audio. Ship a
  stock ElevenLabs voice in the square.
...
The story (Issue 1, Open Progress, For the Record, …)
    → VAL8000 little square (types in the box; idle line, smile after the answer)
```
...
The Frequency (sourced news, URL, date)
    → VAL8000 square (types in the box; idle line, smile after the answer)
...
The magazine square does **not** speak. `scripts/val8000_speak.py` is
CLI, later, clone-only. The widget never calls it. **Type now. Voice later.**
