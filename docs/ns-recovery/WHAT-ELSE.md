...
Two live writes. Everything else is scored or refused.

1. **Attack 9D** — growing input and output supports, full complex polarizations, frequency factors kept. Target \(\|\Pi_\beta B\|_2\le C\alpha\beta^{-1/2}\|w\|_2^2\). Fixed-output \(\Theta(m^2)\) is excluded. This is the remaining packet falsifier.
2. **The ★ reason** — why signed stretching cannot outrun spectral spread. HH→L is the dangerous channel. That sentence is not written. Until it is, or until \(\mathcal R_\star\to\infty\) on a family, Lemma★ stays OPEN.
