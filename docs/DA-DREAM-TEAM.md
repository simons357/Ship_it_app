...
- **B:** B4c on CONC, energy-class \(T\) on SPREAD, B8 clock. Glue sketch written. Frozen low-\(j_*\) has an energy ceiling. Climbing: slow dies, fast sits. \(t=0\) and short \(n=32\) do not produce \(c=8\). Geometry: identity pass; CONC not depleted; CF conditional. Stretching budget: aligned cap pays; time does not empty it. Live: B5b (tube geometry). The PDE is not being tuned. Regularity stays open.
