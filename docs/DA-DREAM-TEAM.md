...
- **B:** B4c on CONC, energy-class \(T\) on SPREAD, B8 clock. Glue sketch written. Frozen low-\(j_*\) has an energy ceiling. Climbing: slow dies, fast sits. NS did not force a saving \(c\) (B11d). Climb sketch is not an a priori (B11e). Longer \(n=32\) past room time did not produce \(c=8\) (B13e). Geometry: identity pass; CONC not depleted; CF conditional. Stretching budget is not an a priori (B15e). Fluids: net cubic cancels; visc owns this ensemble; \(L^2\) is not BKM. Angular \(1/r^2\) does not dominate \(I_{\mathrm{tube}}\). Coherent CONC: signed-strain blob nets; working-box cubic not live. Field occupation: clock stays CONC. Field glue: \(j_*=2\) model grows, NS packet falls. DNS is not an a priori (B13f). Tube budget is not an a priori (B5f). Geometry is not an a priori (B14d). Enstrophy balance is not an a priori (B16e). Coherent blob is not an a priori (B17e). Field occupation is not an a priori (B18e). Field glue is not an a priori (B19e). NS climb is not an a priori (B20e). Climb sketch is not an a priori (B21e). Finer box is not an a priori (B22e). Finer DNS is not an a priori (B23e). Leftover close is not an a priori (B34e). Regularity leftover is not an a priori (B35e). Regularity stays open. The PDE is not being tuned.
