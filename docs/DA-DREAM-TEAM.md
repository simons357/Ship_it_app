...
- **B:** B4c on CONC, energy-class \(T\) on SPREAD, B8 clock. Glue sketch written. Frozen low-\(j_*\) has an energy ceiling. Climbing: slow dies, fast sits. \(t=0\) and short \(n=32\) do not produce \(c=8\). Geometry: identity pass; CONC not depleted; CF conditional. Stretching budget: aligned cap pays a leftover. Fluids: net cubic cancels; visc owns this ensemble; \(L^2\) is not BKM. Angular \(1/r^2\) does not dominate \(I_{\mathrm{tube}}\). Coherent CONC: signed-strain blob nets; working-box cubic not live. Field occupation: clock stays CONC; did not save \(X\); cubic not live in time. Live: glue sketch as an NS a priori (B9d). The PDE is not being tuned. Regularity stays open.
