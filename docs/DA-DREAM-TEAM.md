...
- **B:** B4c on CONC, energy-class \(T\) on SPREAD, B8 clock. Glue sketch written. Frozen low-\(j_*\) has an energy ceiling. Climbing: slow dies, fast sits. NS did not force a saving \(c\) (B11d). Geometry: identity pass; CONC not depleted; CF conditional. Stretching budget: aligned cap pays a leftover. Fluids: net cubic cancels; visc owns this ensemble; \(L^2\) is not BKM. Angular \(1/r^2\) does not dominate \(I_{\mathrm{tube}}\). Coherent CONC: signed-strain blob nets; working-box cubic not live. Field occupation: clock stays CONC. Field glue: \(j_*=2\) model grows, NS packet falls. Live: climb sketch as an NS a priori (B11e). The PDE is not being tuned. Regularity stays open.
