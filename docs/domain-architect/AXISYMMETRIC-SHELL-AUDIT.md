...
- Class hunt executed (2026-09-16): KEEP/KILL on pure-swirl / finite-\(K_{\max}\) / small-data / OOE / spectral-gap / \(\theta\le\theta_\ast\) / sparse / meridional — see estimate § Class hunt; harness `scripts/axisym_class_hunt.py`. No Millennium-scaling large-data seat of (A). Sharpest survivor: enforced \(\theta\) (bridge to (A) missing). OOE **KILL** as general seat. Dream-team ranking card: [`DA-CLASS-HUNT-BRIEF.md`](DA-CLASS-HUNT-BRIEF.md). **(A) not seated.** DA-VC-01 stays **FAIL**.
