...
**Recorded 2026-09-16 (same-scale \(T_{j\leftarrow j}\) attack, not a close).** Class: unaugmented axisymmetric-with-swirl. Quantity: Door-1 same-scale block. Remainder: \(T_{j\leftarrow j}\). Assumed: [signed Im triad form; small exact disks; no \(\dot e_j/\dot Z/\Lambda'\)]. Structured object; HH→L separated from same-scale. Sharp \(b=0\) energy internal transfer \(\equiv 0\) by triad pairing (identity, **not** depletion). Door-1 remainder is near-scale \(b\ge 1\) / enstrophy. Avenues: axisym cancellation PARTIAL; depletion⇒(A) false candidates KILLED (occupancy/\(\alpha\)/recycling/\(b=0\)-as-depletion); conditional \(\theta\)-bound PARTIAL (not (A)); numeric search PARTIAL (near-scale nonzero on axisym disks). Status stays **OPEN**. Clay is **NOT CLAIMED**. DA-VC-01 stays **FAIL**.

**Recorded 2026-09-16 (DA dream-team class hunt, not a close).** Class: unaugmented axisymmetric-with-swirl plus named subclasses. Quantity: Door-1 / near-scale \(T_{j\leftarrow j}\). Remainder: \(T_{j\leftarrow j}\). Assumed: [DA structural consult only; no invented proofs; no stack merge with Lemma★]. Ranked routes: (1) **TRY** \(\theta\)-class bridge to (A); (2) **TRY** sparse/near-scale-free support; (3) **KEEP** pure-swirl / small-data / extra symmetry as conditional only — **KILL** as mixed-class close. Axisym alone failed mixed fields because \(T^{\mathrm{mm}}\) + near-scale \(b\ge 1\) survive. Seating (A) = \(\lvert T_{j\leftarrow j}\rvert\le\varepsilon\nu P_j+R\) without \(\dot e_j/\dot Z/\Lambda'\). Card: [`DA-CLASS-HUNT-BRIEF.md`](DA-CLASS-HUNT-BRIEF.md). (A) **not** seated. Status stays **OPEN**. Clay is **NOT CLAIMED**. DA-VC-01 stays **FAIL**.

**Recorded 2026-09-16 (class hunt executed, not a close).** Class: named subclasses of unaugmented axisymmetric-with-swirl. Quantity: Door-1 near-scale \(T_{j\leftarrow j}\) / route (A). Remainder: \(T_{j\leftarrow j}\). Assumed: [exact-disk probes; signed Im; no \(\dot e_j/\dot Z/\Lambda'\)]. KEEP-CONDITIONAL: \(\theta\le\theta_\ast\), spectral gap, sparse support, small data, fixed \(K_{\max}\). KEEP instantaneous / KILL evolutionary: pure swirl. **KILL** as general seat: OOE / extra reflection (live parity keeps \(O(1)\) near-scale). (A) seats only on trivial/conditional subclasses (pure-swirl identity, small data, fixed disk) — **not** for large-data mixed axisym-with-swirl. Sharpest survivor: enforced \(\theta\) (bridge to palinstrophy (A) missing). Estimate § Class hunt; harness `scripts/axisym_class_hunt.py`. Status stays **OPEN**. Clay is **NOT CLAIMED**. DA-VC-01 stays **FAIL**.

**Recorded 2026-09-16 (dead-end / KILL method lock, not a close).** Jonathan Simons standing rule for the axisym / DA stack: **what doesn’t work is almost as important as what actually does work.** Dead-end / **KILL** documentation is a **first-class deliverable** alongside **KEEP**. Class-hunt briefs list **Dead ends first**, then survivors. No invented math; no fake close. File: [`DA-CLASS-HUNT-BRIEF.md`](DA-CLASS-HUNT-BRIEF.md). Estimate honesty: [`docs/papers/swirl/AXISYMMETRIC-SHELL-ESTIMATE.md`](../papers/swirl/AXISYMMETRIC-SHELL-ESTIMATE.md). Status stays **OPEN**. Clay is **NOT CLAIMED**. DA-VC-01 stays **FAIL**.

**Recorded 2026-09-14 (A)–(C) test + conditional Gronwall repair + occupancy, not a close).** Class: unaugmented axisymmetric-with-swirl. Quantity: labeled shells. Remainder: \(T_{j\leftarrow j}\). Assumed: [Jon’s proof-chain (A)/(B)/(C) filed; no invented proofs]. (A)(B)(C) tested as diagnostics only: (A) class inconclusive; (B) fail as depletion⇒(A) on occupancy 1 + \(\alpha\approx 1/2\); (C) inconclusive for class close / pass on named pure-swirl samples only. Conditional Gronwall under (A) repaired as a \(\nu^1\) template with explicit hypotheses; \(\nu^2\) T2 pattern refused (cross-link). Occupancy 1 + \(\alpha\approx 1/2\) still not depletion. Status stays **OPEN**. Clay is **NOT CLAIMED**. DA-VC-01 stays **FAIL**.
