...
4. **Axisymmetric-with-swirl shell remainder \(T_{j\leftarrow j}\)** — Class: unaugmented axisymmetric NS with swirl. Quantity: \(Z_j\). Remainder: \(T_{j\leftarrow j}\). Assumed: [no DNS; no closed stepper]. Door-1 pairing is written; leftover is still intra-shell transfer and still **OPEN**. Pairing residual is an algebraic unit check (gate \(10^{-16}\)). \(\rho_j=T_{j\leftarrow j}/Z_j\) is **NOT COMPUTED** here. Class statement (swirl removes free helical HHH) is **not** a measured 3-D CFM close. Clay **NOT CLAIMED**. DA-VC-01 stays **FAIL**.
