...
class Field:
    ...
    def scale(self, a):
        """Return a new Field equal to a * self (a real or complex scalar)."""
        out = Field()
...
def R_star(field: Field, verify=True, tol=1e-6):
    ...
    if Ds_m <= 1e-14:
        ...
        return dict(E=E, X=X, Y=Y, Z=Z, Lambda=Lambda, D_s=Ds_m, T_c=Tc,
                    R_star=(0.0 if Tc_plus == 0.0 else float("inf")),
                    vacuous_single_shell=True)

    Rs = (Tc_plus ** 2) / (Ds_m * E * Y)
    return dict(E=E, X=X, Y=Y, Z=Z, Lambda=Lambda, D_s=Ds_m, T_c=Tc,
                R_star=Rs, vacuous_single_shell=False)
...
def project_onto_shell(field: Field, beta, target_ks=None):
    """
    Return Pi_beta(field): a new Field containing, for every k with
    |k|^2 = beta, the value obtained by evaluating field's associated
    quantity (here used for Pi_beta B(w,w), so `field` here is
    conceptually "the nonlinear term evaluated at shell-beta k's" --
    see build_exact_shell_closing_packet for the actual usage).
    """
    raise NotImplementedError("use build_closing_direction instead; see below")
