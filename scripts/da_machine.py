...
def classify_claim(claim: str) -> dict:
    ...
    if re.search(
        r"bridge|prime.?block|h_n|inverse.?gcd|qtilde|theorem p|"
        r"goldbach|gold box|\bgnc\b|multi.?rep",
        text,
    ):
    ...
    if re.search(
        ...
        text,
    ) and not re.search(r"\bhodge\b", text):
        return {
            "domain": "U",
            "verdict": "open",
            "reason": "looks like BSD; run proof --problem BSD. Rank leftover stays open.",
        }
    if re.search(r"\bhodge\b", text):
        return {
            "domain": "U",
            "verdict": "open",
            "reason": "looks like Hodge; run proof --problem HODGE. Leftover stays open.",
        }
    if re.search(r"poincar|point care|pointcare", text):
        return {
            "domain": "U",
            "verdict": "open",
            "reason": "looks like Poincaré; run proof --problem POINCARE. Literature sits.",
        }
    if re.search(r"\bp\s*(versus|vs\.?)\s*np\b|\bp\s*=\s*np\b", text):
        return {
            "domain": "U",
            "verdict": "open",
    ...
    if re.search(
        ...
        return {
            "domain": "U",
            "verdict": "open",
            "reason": "looks like complete versions; run proof --problem ALL. Emit is not QED.",
        }
    if re.search(
...
def main() -> int:
    ...
    prf.add_argument(
        "--problem",
        default="",
        help="NS | A | RH | YM | BSD | HODGE | POINCARE | PNP | ALL. Empty defaults to NS.",
    )
    sub.add_parser(
        "picture",
