#!/usr/bin/env python3
"""Exact-core gate for localized ABC. Four jobs only.

1. 3-mode triad vs stokes_moments.probe (on that field T_c=0; D_s matches).
2. Export 99% energy modes and sum the triad — λ=2, 3, 4 only.
3. Same truncated field on a grid: FFT T_c = triad to 10^{-13}.
4. v(n·) leaves R★ flat.

Does not run λ=8 or 16. A finite climb is not a falsifier.
Evaluator ≠ proof. Lemma★ still open. NS not solved. Not a plate.
"""
...
def main():
    ...
    payload = {
        ...
        "verdict": (
            "Four-job gate sits. Same truncated field: FFT=triad. "
            "v(n·) leaves R★ invariant. 99% cutoff ≠ full FFT. "
            "Finite climb on λ=2,3,4 only. Not a falsifier. "
            "Lemma★ remains OPEN. NS not solved. H1 untested."
            if same_ok and dil_ok
            else "Exact-core gate failed. Do not stamp Target A. Lemma★ OPEN."
        ),
    }
