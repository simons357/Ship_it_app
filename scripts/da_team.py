...
def consensus() -> dict:
    ...
    return {
        "not_a_vote": True,
        "not_a_close": True,
        "B": "B4c budgets I_tube on CONC; energy-class T on SPREAD; B8 clock. Glue sketch is written. Frozen low-j has an energy ceiling. Climbing: slow dies, fast sits. NS did not force a saving c (B11d). Climb sketch is not an a priori (B11e). Longer n=32 past room time did not produce c=8 (B13e). DNS is not an a priori (B13f). Tube budget is not an a priori (B5f). Geometry is not an a priori (B14d). Stretching budget is not an a priori (B15e). Enstrophy balance is not an a priori (B16e). Fluids: visc owns this ensemble; L² is not BKM. Angular 1/r² does not dominate I_tube. Coherent CONC: signed-strain blob nets; working-box cubic not live. Field occupation: clock stays CONC. Field glue: j*=2 model grows, NS packet falls. Live leftover: coherent CONC (B17e). Finer is B22e. Regularity stays open.",
