...
def consensus() -> dict:
    ...
    return {
        "not_a_vote": True,
        "not_a_close": True,
        "B": "B4c budgets I_tube on CONC; energy-class T on SPREAD; B8 clock. Glue sketch is written. Frozen low-j has an energy ceiling. Climbing: slow dies, fast sits. NS did not force a saving c (B11d). Climb sketch is not an a priori (B11e). Longer n=32 past room time did not produce c=8 (B13e). DNS is not an a priori (B13f). Tube budget is not an a priori (B5f). Geometry is not an a priori (B14d). Stretching budget is not an a priori (B15e). Enstrophy balance is not an a priori (B16e). Coherent blob is not an a priori (B17e). Field occupation is not an a priori (B18e). Field glue is not an a priori (B19e). NS climb is not an a priori (B20e). Fluids: visc owns this ensemble; L² is not BKM. Angular 1/r² does not dominate I_tube. Field occupation: clock stays CONC. Field glue: j*=2 model grows, NS packet falls. Climb sketch is not an a priori (B21e). Finer box is not an a priori (B22e). Finer DNS is not an a priori (B23e). Leftover close is not an a priori (B34e). Regularity leftover is not an a priori (B35e). Regularity stays open.",
