...
WATCH = {
    ...
    "B.B5b_tube_vs_viscosity": {
        ...
        "on_fail": (
            ...
            "Keep B4c (full tube dissipation still budgets the packet). Do not cancel to Φ. Next: NS climb law (B11d).",
        ),
        "on_pass": (
...
def render_text(events: list[dict], baseline: bool) -> str:
    ...
    if not events:
        return (
            ...
            "Next: NS climb law (B11d). Field glue scored. B4c stands. Do not cancel to Φ.\n"
        )
    parts = []
    for e in events:
        sms = f"{e['slot']}: {e['head']}"
...
def notify(
    ...
    payload = {
        ...
        "recommendation": (
            events[-1]["next"]
            if events
            else "Nothing flipped. Next write is the NS climb law (B11d)."
        ),
    }
