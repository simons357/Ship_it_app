...
**Which build won:** Jonathan reversed the morph-glass look lock (25 Aug 2026). Live UI in `chatvault/` is a **steel vault** skin: solid charcoal, sharp type, one amber signal accent, no `backdrop-filter`. Tagline stays **OS for your AI**. The git tree `chatvault/` remains the canonical **engine / data model** (ChatVaultEntry, CLAIM_LEDGER, BM25F fielded retrieval). Glass is archived as a Base44 look, not the shipped chrome.
