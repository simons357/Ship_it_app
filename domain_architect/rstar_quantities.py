...
Shape statement (viscosity cancelled under u=av):

    (T_c(v)_+)^2 <= 4 θ C_0 D_s(v) ‖v‖₂² Y(v)
    R_★(v) = (T_c_+)^2 / (D_s ‖v‖₂² Y)
...
See docs/ns-review/LEMMA-STAR-EXACT-FORMULAS.md.
"""
...
def T_c_from_signed_triads(
    ...
    *,
    ...
    return TcResult(
        ...
        complete=not restricted,
        note=(
            "T_c=M−ΛN from signed T_k (no abs); "
...
def shape_form_holds(
    ...
    return {
        ...
        "proof": False,
        "ns_solved": False,
        "boxed": "(T_c(v)_+)^2 <= 4*theta*C_0*D_s(v)*||v||_2^2*Y(v)",
    }
...
def exact_formula_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "nonlinear": {
            ...
            "N": "sum lambda T_k",
            "M": "sum lambda^2 T_k",
            "T_c": "M - Lambda*N = sum lambda(lambda-Lambda) T_k",
