...
- 9B Exact-shell coherent fan + small closing — LIVE; finite sample max K≈0.641
  at (α,β)=(4,8) is NOT HH→L (β>α = higher-shell); genuine HH→L needs β<α
- 9C Fixed-gap spheres — natural same-shell NOT a kill (0.11→0.031); SoT-only
  until implemented; no sweep script/data in PR #48
...
Canonical quotient name: ratio_R_star_shape. Legacy ratio_star is DIFFERENT — do not compare.
Attack 3 filters HF inputs; does NOT restrict output to low → not strictly HH→L.

See docs/ns-review/FIVE-LANE-BOOKKEEPING.md,
    ...
    docs/ns-review/five-lane-recovery/docs/math/ns_attacks/ATTACK_9B_EXACT_SHELL_CLOSING.md.
"""
...
DOC_PATH_MATH = (
    "docs/ns-review/five-lane-recovery/docs/math/ns_attacks/"
    "ATTACK_9B_EXACT_SHELL_CLOSING.md"
)
...
BOOKKEEPING_DOC = "docs/ns-review/FIVE-LANE-BOOKKEEPING.md"
HEADLINE_PATH = (
    "docs/ns-review/five-lane-recovery/results/ns_five_lane_2026-09-10/"
    "attack9b_exact_shell/HEADLINE.md"
)
...
CANONICAL_QUOTIENT = "ratio_R_star_shape"
LEGACY_QUOTIENT = "ratio_star"
...
FIXED_GAP_R_STAR_ATTESTED: dict[str, Any] = {
    ...
    "trend": "falls_with_n",
    "tracks_m_half": False,
    "probe_status": "SoT_only_until_implemented",
    "pr48_sweep_script": False,
    "pr48_sweep_data": False,
    "note": (
        "0.11→0.031 is this family's R_★ under exact "
        f"{EXACT_R_STAR_FORMULA}; SoT-only — no sweep script/data in PR #48; "
        "not comparable to unattested legacy values or to legacy ratio_star"
    ),
}
...
# Attack 9C = fixed-gap (user SoT). Keep ATTACK_9_FIXED_GAP_STATUS as alias.
ATTACK_9C_STATUS: dict[str, Any] = {
    ...
    "note": (
        ...
        "SoT-only until implemented; no sweep script/data in PR #48."
    ),
    "doc": FIXED_GAP_DOC,
}

ATTACK_9_FIXED_GAP_STATUS = ATTACK_9C_STATUS

ATTACK_9B_STATUS: dict[str, Any] = {
    "id": "ATTACK-9B-EXACT-SHELL-CLOSING",
    "verdict": "LIVE_NUMERIC_NOT_A_KILL",
    "family": "v_eps = w_alpha + eps z_beta",
    "closing": "z_beta parallel Pi_beta B(w_alpha, w_alpha)",
    "quantity": KAB_FORMULA,
    "runtime_max_K": 0.641,
    "runtime_max_pair": (4, 8),
    "runtime_max_is_HH_to_L": False,
    "runtime_max_reason": "beta>alpha is higher-shell transfer; HH→L needs beta<alpha",
    "canonical_quotient": CANONICAL_QUOTIENT,
    "legacy_quotient_do_not_compare": LEGACY_QUOTIENT,
    "controls_pass": True,
    "eps_limit_pass": True,
    "next_clean_test": [
...
ATTACK_9D_STATUS: dict[str, Any] = {
    ...
    "proved_lemma_star": False,
    "note": (
        "Natural same-shell / fixed-gap (9C) did not kill ★. Remaining falsifier: "
        "designed Θ(m²)-closure subset with locked phases. Kill lane LIVE. "
        "Earlier DA notes called this '9C' — renamed to 9D."
    ),
    "doc": THETA_M2_DOC,
}

# Backward-compat alias: old code expected ATTACK_9C_STATUS = Θ(m²). Prefer ATTACK_9D.
ATTACK_9C_THETA_M2_LEGACY_ALIAS = ATTACK_9D_STATUS

ATTACK_3_PRECISION: dict[str, Any] = {
    "filters_high_frequency_inputs": True,
    "restricts_output_to_low": False,
    "strictly_HH_to_L": False,
    "note": (
        "Attack 3 Bony split tags parent scales; does not force low output — "
        "not strictly HH→L. Kill uses complete signed T_c."
    ),
}
...
def refuse_same_shell_ensemble_kills_star(text: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "kill_lane": dict(KILL_LANE_STATUS),
        "message": (
            "REFUSE: natural same-shell / fixed-gap ensemble (Attack 9C) is NOT "
            "a kill (R_★ falls 0.11→0.031 under exact formula; closures O(m); "
            "does not track m^{1/2}; SoT-only, no PR #48 sweep). Kill lane LIVE "
            "via Attack 9D (designed Θ(m²)-closure subset, locked phases). "
            "NS NOT SOLVED."
            if hit
            else (
...
def kab_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "attack_9c": dict(ATTACK_9C_STATUS),
        "attack_9d": dict(ATTACK_9D_STATUS),
        "attack_3_precision": dict(ATTACK_3_PRECISION),
        "K_alpha_beta": KAB_FORMULA,
        "exact_R_star": EXACT_R_STAR_FORMULA,
        "canonical_quotient": CANONICAL_QUOTIENT,
        "legacy_quotient_do_not_compare": LEGACY_QUOTIENT,
        "docs": {
            "bookkeeping": BOOKKEEPING_DOC,
            "attack_9": ATTACK_9_DOC,
            "attack_9_fixed_gap": FIXED_GAP_DOC,
            "attack_9_fixed_gap_legacy": FIXED_GAP_DOC_LEGACY,
            "attack_9b": DOC_PATH,
        ...
        "refusals": [
            ...
            "9B max K at (4,8) is HH→L",
            "compare ratio_star to ratio_R_star_shape",
        ],
        "jonathan_action": "none",
        "remaining_falsifier": "ATTACK-9D-THETA-M2-CLOSURE",
        "naming": {
            "9A": "AP packet fail",
            "9B": "exact-shell K_alpha_beta",
            "9C": "fixed-gap spheres",
            "9D": "Theta(m^2) locked phases",
        },
    }
